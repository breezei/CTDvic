...
public class eleMHRresult2_dao {
    ...
    public eleMHRresult2_dao(){}
    public void analyse(String relativePathToData,String uuid) throws IOException {
        String readPath = rootPath + relativePathToData + "analysis/results/"+uuid+"/element_mapping/";
        String writePath = rootPath + relativePathToData + "analysis/results/"+uuid+"/element_result/";
        //相比于eleMHRresult2添加循环处理癌症类型中的5个文件，并写出
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            //创建写出文件BufferedWriter对象
            String WriterFile = writePath;
            BufferedWriter bw = new BufferedWriter(new FileWriter(WriterFile+filename));
            //先将标题写出
            bw.write("gene" + "\t" + "p" + "\t" + "nMut" + "\t" + "MES.max" + "\t" + "maxpos" + "\t" + "MES.min" + "\t" + "minpos" + "\t" + "MES" + "\n");
            ...
            for (String gene : genes) {
                ...
                List<Double> esRandoms = Collections.synchronizedList(new LinkedList<Double>());
                //java7预改：List<Double> esRandoms = Collections.synchronizedList(new LinkedList<>());
                ...
                */
                for (List<Integer> integers : lists) {
                    esRandoms.add(getResult(integers, length, range, start)[4]);
                }
                //计算p值
                //nes 0ms
