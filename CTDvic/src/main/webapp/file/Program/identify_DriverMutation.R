...
for (i in 1:length(list)) {
  element_filter<-read.table(paste(readTablePath_element_filter,list[i],sep = ""),
  ...
  if(nrow(driver_mutation)>0){
    write.table(driver_mutation,paste(writeTablePath,list[i],sep = ""),sep="\t",quote=FALSE,row.names = F)
  }
